                    ...
                    <div class="container-fluid px-5">
                        <div class="row my-5">
                            <div class="d-flex align-items-center mb-3">
                                <a href="{{ route('admin.orders.index') }}" class="text-decoration-none btn btn-secondary btn-sm me-3">
                            ...
                            <div class="col-xl-6 col-md-6 mt-3 border border-primary p-3"> 
                                ...
                                <h6 class="text-dark">
                                    ...
                                    @else
                                        ...
                                        {{ $order->user->address ?? '123 Main Street' }}<br>
                                        {{ $order->user->city ?? 'City' }}, {{ $order->user->state ?? 'State' }}
                                    @endif
                                </h6>
                                <span class="text-dark"><strong>Email:</strong> {{ $order->user->email ?? 'sandesh@example.com' }}</span><br>
                                <span class="text-dark"><strong>Phone:</strong> {{ $order->user->phone ?? '(123) 456-7890' }}</span>
                            </div>
                            <div class="col-xl-6 col-md-6 mt-3 border border-primary p-3"> 
                                <h5 class="text-dark">Order Summary</h5>
                                <p class="text-dark"><strong>Order Id:</strong> {{ $order->order_id ?? '2001' }}</p>
                                <p class="text-dark"><strong>Payment Method:</strong> {{ ucfirst($order->payment_method ?? 'cod') }}</p>
                                <p class="text-dark"><strong>Sub Total:</strong> Rs. {{ number_format($order->total ?? 100.00, 2) }}</p>
                                <p class="text-dark"><strong>Discount:</strong> Rs. 0.00</p>
                                <p class="text-dark"><strong>Shipping Fee:</strong> Rs. 10.00</p><hr>
                                <h5 class="text-dark mt-3">Order Total: Rs. {{ number_format(($order->total ?? 100.00) + 10.00, 2) }}</h5>
                                <div class="mt-2">
                                    ...
                                    <tbody>
                                        ...
                                        @else
                                            ...
                                            <td>
                                                <div class="d-flex">
                                                    <div>    
                                                        <img src="{{ asset('assets/images/Product/1.png') }}" class="img-fluid rounded" style="width: 70px;" alt="Combo Gift" onerror="this.src='{{ asset('assets/images/Product/1.png') }}'">
                                                    </div>
                                                    <div class="p-3"><h6>Combo Gift</h6></div>
                                                </div>
                                            </td>
                                            <td>Rs. 100</td>
                                            <td>01</td>
